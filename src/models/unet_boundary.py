"""
Boundary-Aware Residual U-Net Architecture for Precise Waterbody Delineation
...
class BoundaryAwareResUNet(nn.Module):
    def __init__(self, in_channels: int = 6, num_classes: int = 1):
        """
        Dual-head network: Produces surface water masks alongside edge/boundary masks
        to penalize shoreline fuzziness and mixed-pixel uncertainty.
        """
        super().__init__()
        # Encoder
    ...
    def forward(self, x: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        # Outputs
        water_mask = self.water_head(d1)
        boundary_mask = self.boundary_head(d1)

        return water_mask, boundary_mask
